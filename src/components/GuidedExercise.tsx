...
import { ChevronDown, ChevronRight, Eye, CheckCircle2, Lightbulb, ArrowRight } from 'lucide-react'
...
export function GuidedExercise({ exercise, onComplete }: GuidedExerciseProps) {
  ...
  const handleShowHint = (stepIndex: number) => {
    if (!showHints.includes(stepIndex)) {
      setShowHints([...showHints, stepIndex])
    }
  }

  const handleShowSolution = (stepIndex: number) => {
    if (!showSolutions.includes(stepIndex)) {
      setShowSolutions([...showSolutions, stepIndex])
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {exercise.steps.map((step, index) => {
          ...
          const hasHint = showHints.includes(index)
          const hasSolution = showSolutions.includes(index)
