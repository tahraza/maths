...
export function ProgressBackup() {
  ...
  const handleExport = () => {
    ...
    a.href = url
    a.download = `maths-sauvegarde-${new Date().toISOString().split('T')[0]}.json`
    ...
    reader.onload = (e) => {
      try {
        ...
        if (data.gamificationStore) {
          ...
          localStorage.setItem('maths-gamification', JSON.stringify(gamificationData))
        }
        ...
        if (data.petStore) {
          ...
          localStorage.setItem('maths-pet', JSON.stringify(petData))
        }
        ...
        if (data.challengesStore) {
          ...
          localStorage.setItem('maths-challenges', JSON.stringify(challengesData))
        }
  ...
  const handleReset = () => {
    if (confirm('Êtes-vous sûr de vouloir réinitialiser toute votre progression ? Cette action est irréversible.')) {
      ...
      localStorage.removeItem('maths-store')
      localStorage.removeItem('maths-gamification')
      localStorage.removeItem('maths-pet')
      localStorage.removeItem('maths-challenges')
